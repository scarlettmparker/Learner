...
Rules (delegated, read the granular skills for detail):
 ...
 - Balance and size: see `quiz-balance`. For basic and same roughly one third each. For advanced and mastery, skew to half short, quarter mcq, quarter fill. Variable length 5 basic/easy, 10 same/normal, 20 advanced/mastery. You must count and push to the full number, don't stop at 4 when 10 was requested; accept less only if you truly run out of spans. Env `LLM_REASONING_EFFORT` and `LLM_MAX_OUTPUT_TOKENS=8192` give you room. mcq needs 4 distinct full phrases, all same type as answer, correct spread across A to D, no placeholder "A" or "Not ...1", no option echoing the stem past five chars, never put the answer in parentheses inside an option, and Answer must be bare phrase never "A. phrase" or "B. phrase" (grading compares the phrase after shuffling, a prefix breaks it).
